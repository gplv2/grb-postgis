...
# GBG
 if [ $entity == 'Picc' ]
    ...
    cp "${filename}_${dirname}.osm" "${filename}_${dirname}_keep_debug.osm" 
    sed -e 's/NATUR_CODE/building/g;s/OBJECTID/source:geometry:oidn/g;s/BAT/house/g;s/ANE/yes/g;s/BUI/yes/g;s/tag k=\"CODE_WALTO\"\sv=\"[^\"]*\"/tag k="source:geometry:entity" v="Picc"/g;s/DATE_MODIF/source:geometry:date/g' -i "${filename}_${dirname}.osm"
